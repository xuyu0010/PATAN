...
class StaticGradReverse(Function):
	@staticmethod
...
# Dynamic gradient reverse layer re-implement with new Function
class DynamicAdversarialLayer(Function):
	@staticmethod
	def forward(ctx, x, high_value, pass_num):
		ctx.pass_num = pass_num
		ctx.high = high_value
		ctx.alpha = 1.0
		ctx.low = 0.0
		ctx.max_iter = 1000.0
		ctx.beta = np.float(2.0 * (ctx.high - ctx.low) / (1.0 + np.exp(-ctx.alpha * ctx.pass_num / ctx.max_iter)) - (ctx.high - ctx.low) + ctx.low)
		return x.view_as(x)
...
class AdversarialNetwork(nn.Module):
	def __init__(self, in_feature, mid_feature):
		super(AdversarialNetwork, self).__init__()
		self.ad_layer1 = nn.Linear(in_feature, mid_feature)
		self.ad_layer2 = nn.Linear(mid_feature, mid_feature)
		self.ad_layer3 = nn.Linear(mid_feature, 1)
...
class DannDomainNetwork(nn.Module):
	def __init__(self, in_feature):
		super(DannDomainNetwork, self).__init__()
		self.ad_layer0 = nn.Linear(in_feature, in_feature)
		self.ad_layer1 = nn.Linear(in_feature, 2)
		self.ad_layer0.weight.data.normal_(0, 0.01)
		self.ad_layer1.weight.data.normal_(0, 0.01)
		self.ad_layer0.bias.data.fill_(0.0)
		self.ad_layer1.bias.data.fill_(0.0)
