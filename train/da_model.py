...
from easydl import EntropyLoss
...
class static_model(object):
	...
	def __init__(self, net, model_prefix='', da_method=None, use_spatial=False, **kwargs):
		...
		self.pata_tradeoff = 1.0
		self.dann_tradeoff = 1.0
		self.aux_clip_tradeoff = 0.5
		self.ent_tradeoff = 0.2
		self.use_tgt_ent_loss = True
		self.spatial_tradeoff = 0.5
		self.pata_sp_tradeoff = 0.5
	...
	def forward(self, src_data, tgt_data, src_label, tgt_label, class_weight, step_count):
		""" typical forward function with:
			single output and single loss
		"""
		if self.net.module.__class__.__name__ == 'PATAN':
			...
			_,tgt_feat,src_pred,tgt_pred,src_sp_pred,tgt_sp_pred,_,tgt_scale_preds,src_scale_preds_raw,_,src_adv,tgt_adv,src_sp_adv,tgt_sp_adv = self.net(src_data, tgt_data, step_count)
			src_output = src_pred
			tgt_output = tgt_pred
			adv_all = torch.cat((src_adv, tgt_adv), dim=0)
			adv_sp_all = torch.cat((src_sp_adv, tgt_sp_adv), dim=0)

		else:
			logging.error("network '{}'' not implemented".format(self.net.module.__class__.__name__))
		...
		if src_label is not None:
			if self.net.training:
				...
				loss = class_criterion(src_output, src_label)
				if self.use_spatial:
					loss_sp = class_criterion(src_sp_pred, src_label)

				if self.use_tgt_ent_loss and 'PATAN' in self.net.module.__class__.__name__:
					tgt_entropy = EntropyLoss(nn.Softmax(dim=-1)(tgt_pred))
					loss += self.ent_tradeoff * tgt_entropy
					losses.append('tgt_entropy: ' + str(tgt_entropy.item()))

				# auxiliary class classification for clip features
				if self.net.module.__class__.__name__ == 'PATAN':
					loss_aux_clip = 0
					for j in range(len(src_scale_preds_raw)):
						loss_aux_clip += clip_criterion(src_scale_preds_raw[j], src_label)
					loss += self.aux_clip_tradeoff * loss_aux_clip
					losses.append('loss_aux_clip: ' + str(loss_aux_clip.item()))

				# transfer losses (Note: domain classification: 1 for source and 0 for target)
				if self.da_method == 'PATA':
					loss_pata = metric.PATA(input_all, adv_all, src_label, class_weight)
					loss += self.pata_tradeoff * loss_pata
					losses.append('loss_pata: ' + str(loss_pata.item()))
					if self.use_spatial:
						loss_pata_sp = metric.PATA(input_all, adv_sp_all, src_label, class_weight)
						loss += self.pata_sp_tradeoff * loss_pata_sp
						losses.append('loss_sp_pata: ' + str(loss_pata_sp.item()))

				elif self.da_method == 'DANN':
					loss_dann = metric.DANN(src_data, tgt_data, adv_all)
				...
				if torch.cuda.is_available():
					class_criterion = class_criterion.cuda()
				loss = class_criterion(tgt_output, tgt_label)
		else:
			loss = None
			losses = []

		if 'PATAN' in self.net.module.__class__.__name__:
			return [tgt_output], [src_output], [tgt_sp_pred], tgt_scale_preds, _, [loss], losses, [tgt_feat]
		else:
			return [tgt_output], [src_output], [loss], losses, [tgt_feat]
...
class da_model(static_model):
	...
	def __init__(self,net,model_prefix='',da_method=None,num_classes=400,use_spatial=False,
				 step_callback=None,step_callback_freq=50,step_class_w_update=100,start_clip_weight=0,epoch_callback=None,save_freq=1,batch_size=None,**kwargs):
		...
		self.start_clip_weight = start_clip_weight
	...
	def fit(self, src_train_iter, tgt_train_iter, optimizer, lr_scheduler, tgt_eval_iter=None, 
		...
		for i_epoch in range(epoch_start, epoch_end):
			...
			elif src_train_iter.__len__() < tgt_train_iter.__len__():
			...
			for i_batch, ((src_data, src_label), (tgt_data, tgt_label)) in train_iter:
				...
				dynamic_weight_reference = torch.zeros(tgt_data.shape[0], self.num_classes)
				...
				if 'PATAN' in self.net.module.__class__.__name__:
					...
					if (step_count > self.start_clip_weight) and (not torch.equal(dynamic_weight_reference.data.cpu(), outputs_scale_tgt[0].data.cpu())) and first_dynamic_weight:
						first_dynamic_weight = False
						logging.info("Training:: Start using dynamic clip weight at step {}".format(step_count))
				else:
					outputs_tgt, outputs_src, losses, each_losses, _ = self.forward(src_data, tgt_data, src_label, tgt_label, class_weight, step_count)
				...
				elif (step_count % self.step_class_w_update) == 0 and self.da_method == 'PATA' and 'PATAN' in self.net.module.__class__.__name__:
					logging.info("Updating class weight for PATA")
					start_update_proc = True
					self.net.eval()
					assert update_iter is not None
					for j_batch, (tgt_data_update, tgt_label_update) in update_iter:
						preds_up, _, preds_up_sp, w_scale_preds_up, _, _, _, _ = self.forward(tgt_data_update, tgt_data_update, tgt_label_update, tgt_label_update, class_weight, step_count)
						preds_up = preds_up[0]
						preds_up = nn.Softmax(dim=1)(softmax_param * preds_up)
						preds_up_sp = preds_up_sp[0]
						preds_up_sp = nn.Softmax(dim=1)(softmax_param * preds_up_sp)
						...
						if self.use_spatial:
							all_preds_up = torch.cat((all_preds_up, preds_up_sp.data.cpu().float()), 0)
						for w_scale_pred in w_scale_preds_up:
							all_preds_up = torch.cat((all_preds_up, w_scale_pred.data.cpu().float()), 0)
					
					class_weight = torch.mean(all_preds_up, 0)
					class_weight = (class_weight / torch.mean(class_weight)).cuda().view(-1)
					logging.info("Update class weight for PATA complete")
					self.net.train()
			...
			if (eval_iter is not None) and ((i_epoch+1) % max(1, int(self.save_freq/2))) == 0:
				...
				for i_batch_eval, (tgt_data_eval, tgt_label_eval) in eval_iter:
					...
					if 'PATAN' in self.net.module.__class__.__name__:
						outputs_tgt_eval, _, _, _, _, losses_eval, _, _ = self.forward(tgt_data_eval, tgt_data_eval, tgt_label_eval, tgt_label_eval, class_weight, step_count)
					else:
						outputs_tgt_eval, _, losses_eval, _, _ = self.forward(tgt_data_eval, tgt_data_eval, tgt_label_eval, tgt_label_eval, class_weight, step_count)
